...
#	plot layout dimensions if multiple plots (plotDim)
...
randPlot=function(results,coverage=.95,plotDim=c(length(results$obs_stat),1)){
	interval=randInterval(results,coverage)
	if(length(results$obs_stat)==1){
		par(mfrow=plotDim)
		hist(results$perm_stats,breaks=20,xlab="Test Statistic",main="Randomization Distribution of Test Statistic")
		...
		for(j in 1:length(results$obs_stat)){
			hist(results$perm_stats[,j],breaks=20,xlab="Test Statistic",main=paste("Randomization Distribution of Test Statistic",j))
