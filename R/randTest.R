...
#input: outcomes (y), assignments (w), desired number of permutations (nperm),
...
randTest=function(y,w,nperm,calcTestStat,calcOptions=NULL,calcPO=zeroEffect,poOptions=NULL,
			randOptions=list(type=c("complete","block","Latin"),
		      block = NULL, row = NULL, col = NULL),null=NULL,
			alternative=c("two.sided", "greater", "less")){
	alternative=match.arg(alternative)
	randOptions$type=match.arg(randOptions$type,c("complete","block","Latin"))
	...
	#each list element is a permutation
	if(randOptions$type=="complete"){
		wperm=completeRand(w,nperm)
	}else if(randOptions$type=="block"){
		wperm=blockRand(w,nperm,randOptions$block)
	}else if(randOptions$type=="Latin"){
		wperm=latinRand(w,nperm,randOptions$row,randOptions$col)
	}

	#potential outcomes and corresponding test statistics for each permutation
	if(is.null(calcOptions)){
		if(is.null(poOptions)){
			perm_stats=sapply(1:nperm, function(j) calcTestStat(calcPO(y,w,wperm[[j]]),wperm[[j]]))
		}else{
			perm_stats=sapply(1:nperm, function(j) calcTestStat(calcPO(y,w,wperm[[j]],poOptions),wperm[[j]]))
		}
	}else{
		if(is.null(poOptions)){
			perm_stats=sapply(1:nperm, function(j) calcTestStat(calcPO(y,w,wperm[[j]]),wperm[[j]],calcOptions))
		}else{
			perm_stats=sapply(1:nperm, function(j) calcTestStat(calcPO(y,w,wperm[[j]],poOptions),wperm[[j]],calcOptions))
		}
	}
	...
	if(length(obs_stat)==1){
		if(alternative=="two.sided"){
			pvalue=sum(abs(perm_stats-null)-abs(obs_stat-null)>=0)/nperm
		}else if(alternative=="less"){
			pvalue=sum(perm_stats-obs_stat<=0)/nperm
		}else if(alternative=="greater"){
			pvalue=sum(perm_stats-obs_stat>=0)/nperm
		}
	}else{
		if(alternative=="two.sided"){
			pvalue=rowSums(abs(perm_stats-null)-abs(obs_stat-null)>=0)/nperm
		}else if(alternative=="less"){
			pvalue=rowSums(perm_stats-obs_stat<=0)/nperm
		}else if(alternative=="greater"){
			pvalue=rowSums(perm_stats-obs_stat>=0)/nperm
		}
	}
